...
\begin{figure}
\centering
\includegraphics[width=0.95\textwidth]{ivoa-arch-dm.pdf}
\caption{Data Model Standards and Dependencies}
\label{fig:dmdeps}
\end{figure}

\subsection{VO-DML - REC}

The VO Data Modelling Language (VO-DML) \citep{2018ivoa.spec.0910L} defines a standard 
modelling language, or meta-model, for expressing data models in the IVOA. 
Adopting such a uniform language for all models allows 
...
also from other contexts, in particular from the IVOA standard formats such as VOTable. 
...
VO-DML is limited to the description of static data structures. It uses only the elements of 
the UML language describing the "class diagrams". 
Its concepts can 
...
\subsection{CharDM - REC} 

The Characterisation Data Model (CharDM) \citep{2008ivoa.spec.0325L} defines the high 
level metadata necessary to describe the 
...
\subsection{ObsCoreDM - REC}

The Observation Data Model Core Components (ObsCoreDM) \citep{2017ivoa.spec.0509L} 
specifies the metadata that are necessary to perform data 
...
STC and certain other parts of the Data Model. Two serializations are proposed: XML (STC-X) and
ascii string (STC-S); the former is an integral part of the model.
STC has been refactored in 2 separate components: Meas and Coord (see below)

\subsection{Coords - PR}

The Coordinates Data Model covers the following concepts: description of single and multi-dimensional 
coordinate space and coordinates within that space, coordinate frames providing metadata describing the 
...
extended by other models describing specific types of Datasets and Data products. It specifies 
the generic Dataset, as well as an ObservationDataset model 
...
The MANGO model attaches identifiers on astronomical sources and associates to each a flexible set of parameters
(e.g. observed physical quantities) and other information like e.g. spectra, time series or preview images.

\subsection{ObsLocTAP - PR} 

The Observation Locator Table Access Protocol (ObsLocTAP) defines a data model for scheduled observations 
...
\subsection{EPN-TAP - V2 Draft}

The Euro Planetary Network TAP \citep{2014A&C.....7...52E} framework describes use of TAP with the EPNCore metadata dictionary. 
The EPNCore 
...
metadata dictionaries are also included. 
EPN-TAP 1.0 hasn't been published as a formal VO standard, V2 is proposed as a draft at the time of writing. 
